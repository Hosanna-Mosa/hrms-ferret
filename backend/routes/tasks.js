...
router.get('/me', auth, async (req, res) => {
  try {
    const tasks = await Task.find({ employee_id: req.user.employeeId })
      .sort({ due_date: 1 })
      .exec();
    res.json(tasks);
  } catch (error) {
...
router.post('/', auth, async (req, res) => {
  const { title, sprint, due_date, story_points, status, priority, employee_id } = req.body;
  try {
...
router.get('/sprints/current', auth, async (req, res) => {
  try {
    ...
    if (stats.length > 0) {
      ...
      res.json({ sprint: 'Sprint 12', total_points: 23, completed_points: 18 });
    }
  } catch (error) {
